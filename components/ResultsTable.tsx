...
import { Search, Download, Filter, ExternalLink, Copy, Check, AlertTriangle, XCircle, Mail } from 'lucide-react';
...
const ResultsTable: React.FC<ResultsTableProps> = ({ data }) => {
  ...
  const exportCSV = () => {
    const headers = ["Entreprise", "Email", "Statut", "Dirigeant", "Site Web", "Ville", "Secteur", "Score"];
              ...
              <tr key={company.id} className={`hover:bg-slate-700/30 transition-colors ${selectedIds.has(company.id) ? 'bg-primary-500/5' : ''}`}>
                ...
                <td className="p-4">
                  ...
                  {company.emails.length > 0 ? (
                    <div className="flex items-center gap-2 text-slate-400 mt-1 cursor-pointer hover:text-primary-400 group" onClick={() => copyToClipboard(company.emails[0].address)}>
                      <Mail size={12} />
                      <span className="truncate max-w-[180px]">{company.emails[0].address}</span>
